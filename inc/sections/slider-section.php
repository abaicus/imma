				...
				<div class="carousel-inner" role="listbox">
					...
					<?php if ( ! empty( $imma_slider_arrows_link ) ) { ?>
						<div class="row text-center arrows-wrap">
							<a href="<?php echo esc_url( $imma_slider_arrows_link ); ?>"><i
									class="scroll-invite animated bounce fa fa-3x  fa-angle-double-down"></i></a>
						</div>
					<?php } ?>
				</div>
			</div>
...
<div class="item <?php if ( $count === 1 ) {
	...
	<?php
	if ( ! empty( $image_url ) ) { ?>
	<div class="item-content-wrap" style="background-image: url('<?php echo esc_url( $image_url ) ?>')">
		<?php
		} else { ?>
						...
						<div class="col-sm-10 col-sm-offset-1">
							...
							if ( ! empty ( $button_link ) && ! empty( $button_text ) ) { ?>
								<a href="<?php echo esc_url( $button_link ); ?>"
								   class="btn btn-yellow btn-lg"><?php echo esc_html( $button_text ); ?></a>
							<?php } ?>
						</div>
					</div>
				</div>
	...
	if ( (bool) $imma_slider_inidicators_hide === false && $count !== 1 ) {
		$counter = 0; ?>
		<!-- Indicators -->
		<ol class="carousel-indicators">
			<?php foreach ( $imma_slider_content as $slider_item ) {
				?>
				<li data-target="#myCarousel"
				    data-slide-to="<?php echo esc_attr( $counter ) ?>" <?php if ( $counter === 0 ) {
