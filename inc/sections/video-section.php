...
if ( ! function_exists( 'imma_video' ) ) {
	...
	function imma_video() {
		...
		$video_link      = get_theme_mod( 'imma_video_link' );
		$video_thumbnail = preg_match( '/\/\/(www\.)?(youtu|youtube)\.(com|be)\/(watch|embed)?\/?(\?v=)?([a-zA-Z0-9\-\_]+)/', $video_link, $youtube_matches );
		...
		<section id="video" class="video">
				...
				<div class="row">
					<div class="col-lg-12 text-center">
						<?php if ( ! empty( $imma_video_text ) ) { ?>
							<div
								class="video-section-content"><?php echo wp_kses_post( force_balance_tags( $imma_video_text ) ); ?></div>
						<?php } ?>
					</div>
				</div>
			</div>
			...
			<div class="video-player"
			     data-property="{
			     videoURL:'<?php echo esc_url( $video_link ); ?>',
